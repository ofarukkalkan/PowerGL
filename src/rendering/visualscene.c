...
static void build_geometry_triangle_color( powergl_rendering_geometry *obj, collada_elem( triangles ) *triangles, collada_elem( input_local_offset ) *color_input ) {
	...
	if ( srcAcc->r_float_array.ptr != NULL ) {
		...
		if ( stride == 0 ) stride = 1; // stride is must greater than 0 and default is 1
		...
		for ( size_t i = 0; i < vertex_count ; i++ ) {
			...
			if ( offsets[ 0 ] > -1 ) obj->color[ i ].x = float_arr->_ext.value[ index * stride + offsets[0] ];
			if ( offsets[ 1 ] > -1 ) obj->color[ i ].y = float_arr->_ext.value[ index * stride + offsets[1] ];
			if ( offsets[ 2 ] > -1 ) obj->color[ i ].z = float_arr->_ext.value[ index * stride + offsets[2] ];
		...
		if ( position_input != NULL ) {
			...
			if ( srcPosAcc->r_float_array.ptr != NULL ) {
				...
				for ( size_t i = 0; i < vertex_count ; i++ ) {
					...
					if ( offsets[ 0 ] > -1 ) obj->vertex[ i ].x = float_arr->_ext.value[ index * stride + offsets[0] ];
					if ( offsets[ 1 ] > -1 ) obj->vertex[ i ].y = float_arr->_ext.value[ index * stride + offsets[1] ];
					if ( offsets[ 2 ] > -1 ) obj->vertex[ i ].z = float_arr->_ext.value[ index * stride + offsets[2] ];
...
static void build_camera( collada_elem( node ) * node, powergl_rendering_object *obj ) {
	...
	if ( pers != NULL ) {
		...
		if ( pers->cc_xfov != NULL ) {
			...
			if ( value_ptr != NULL ) cam->xfov = to_radians ( ( float ) ( *value_ptr ) );
		}
		
		if ( pers->cc_yfov != NULL ) {
			value_ptr = ( double * ) pers->cc_yfov->_base.value_ptr;
			if ( value_ptr != NULL ) cam->yfov = to_radians ( ( float ) ( *value_ptr ) );
		}
		
		value_ptr = ( double * ) pers->c_znear->_base.value_ptr;
		if ( value_ptr != NULL ) cam->znear = ( float ) ( *value_ptr );
		
		value_ptr = ( double * ) pers->c_zfar->_base.value_ptr;
		if ( value_ptr != NULL ) cam->zfar = ( float ) ( *value_ptr );
		
		value_ptr = ( double * ) pers->c_aspect_ratio->_base.value_ptr;
		if ( value_ptr != NULL ) cam->aspect_ratio = ( float ) ( *value_ptr );
		...
		if ( orth->cc_xmag != NULL ) {
			...
			if ( value_ptr != NULL ) cam->xmag = ( float ) ( *value_ptr );
		}

		if ( orth->cc_ymag != NULL ) {
			value_ptr = ( double * ) orth->cc_ymag->_base.value_ptr;
			if ( value_ptr != NULL ) cam->ymag = ( float ) ( *value_ptr );
		}
